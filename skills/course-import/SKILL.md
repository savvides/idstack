...
Before writing the manifest, generate a human-readable report at `.idstack/reports/course-import.md` so the designer has a single document to read about what came in and where the quality flags are. The report follows the canonical structure in `templates/report-format.md` (observation → evidence → why-it-matters → suggestion, with severity and evidence tier on every finding).

```bash
mkdir -p .idstack/reports
```

Write `.idstack/reports/course-import.md` with this structure:
...
**Skill:** /idstack:course-import
**Source:** [cartridge | paste | canvas-api | pdf | scorm]
**Source LMS:** [canvas | blackboard | moodle | d2l | rise | storyline | unknown]

## Summary

[2–3 sentences. What came in (N modules, M objectives, P assessments), how many
quality flags were raised, and the single most important thing the designer should
know about the import — e.g., "Import is structurally clean but objectives are
sparse." or "Cartridge schema is valid but 8/12 modules are missing rubrics."]
...
| Objectives | M |
| Assessments | P |
| Activities/discussions | Q |
| Pages | R |
| Rubrics | S |

## Findings

[One block per finding. Stable ids: `import-1`, `quality-1`, `bloom-1`, etc.
Findings come from: import quality flags (Step 2), missing rubrics, missing
objectives, alignment-already-broken-on-arrival, Bloom's-inference low-confidence
classifications.]

### Finding quality-1: [short title]                        [severity] [tier]

**What we saw.** [Concrete observation: "12 modules imported; only 4 have stated
learning objectives."]

**What the evidence says.** [1–2 sentences.] [Domain-N] [Tier]
Example: "Modules without stated objectives leave learners and the assessment
chain underspecified; constructive alignment is harder to evaluate downstream
[Alignment-1] [T5]."

**Why it matters.** [Bridge: missing objectives at the module level mean
downstream skills (assessment-design, course-quality-review) have less to
align against and may flag the gaps as designer intent rather than import gaps.]

**Consider.** [Collaborative recommendation: "Run `/idstack:learning-objectives`
to draft objectives for the 8 modules that are missing them. The Bloom's
classifications already inferred for the 4 existing objectives can serve as
calibration."]
...
[N objectives pre-classified. Note any low-confidence classifications that the
designer should verify.]

| ILO ID | Statement (truncated) | Bloom's level | Confidence |
|--------|----------------------|---------------|------------|
| ILO-1 | ... | analyze | high |

## Top recommendations

1. **[Action]** [Domain-N] [Tier] — [one-line why; pointer to skill]
2. ...

## Limitations

[What this report didn't analyze. Examples: structural metadata only — interactive
elements (Storyline, flashcards) don't render in PDF; Canvas API auth tokens are
never written to disk; SCORM packages with non-standard manifest extensions may
have been parsed loosely.]

## Next steps

1. Run `/idstack:course-quality-review` for the full evidence-based audit.
2. Run `/idstack:learning-objectives` to verify the Bloom's inference and check
   bidirectional alignment.
3. Run `/idstack:needs-analysis` (in audit-existing mode) to add the organizational
   context and learner profile that can't be extracted from the cartridge.

---

*Generated by `/idstack:course-import`. The system-readable manifest section is in `.idstack/project.json` under `import_metadata`.*
```
...
```json
{
  "import_metadata": {
    ...
    "report_path": ".idstack/reports/course-import.md",
...
**Two artifacts:**
- **Read this:** `.idstack/reports/course-import.md` — the import report with
  evidence-backed quality flags, the Bloom's classification table, and recommended
  next steps tied to specific findings.
