...
Before updating the manifest, generate a human-readable build report at `.idstack/reports/course-builder.md` so the designer can see what was generated and why each design choice was made (or, in gap-fill mode, what was skipped and why). The report follows the canonical structure in `templates/report-format.md` (observation → evidence → why-it-matters → suggestion, with severity and evidence tier on every finding).

```bash
mkdir -p .idstack/reports
```

Write `.idstack/reports/course-builder.md` with this structure:
...
**Skill:** /idstack:course-builder
**Mode:** [build-new | gap-fill]
**Expertise adaptation:** [novice | intermediate | advanced | mixed]

## Summary

[2–3 sentences. What was generated, biggest design choice, and any placeholders the
designer needs to fill in before the course is ready for learners.]

## Generated artifacts

| File | Purpose | Notes |
|------|---------|-------|
| syllabus.md | Course-level overview | uses placeholder for due_dates |
| modules/module-01-... | Module 1 content | scaffolding for novices |

## Findings

[One block per finding. Stable ids: `cogload-1`, `multimedia-1`, `placeholder-1`, etc.
Findings come from: cognitive-load decisions made during generation, multimedia-principle
applications and their tradeoffs, places where placeholders were inserted, expertise-
reversal flags from the learner profile that shaped the build.]

### Finding cogload-1: [short title]                        [severity] [tier]

**What we saw.** [Concrete observation: "Module 4 introduces three new frameworks
in the same lesson. Total intrinsic load is high relative to the novice profile."]

**What the evidence says.** [1–2 sentences.] [CogLoad-N] [Tier]

**Why it matters.** [Bridge: working-memory overload during initial schema formation
slows transfer and increases dropout, especially for novices.]

**Consider.** [Collaborative recommendation: "Sequence the three frameworks across
two lessons (4a, 4b) with a worked example bridging them. The current single-lesson
structure is in `module-04-frameworks.md`."]

---

[Repeat per finding. In gap-fill mode, every finding should reference the upstream
flag (e.g., `red-team:alignment-3`) that triggered the generation.]
...
[List of placeholder fields the designer will need to fill in before the course is
ready for learners — instructor name, due dates, institutional links, etc.]

## Limitations

[What this report didn't analyze. Examples: content adaptation is structural, not
voice/style; cognitive-load estimates are heuristic; placeholders are detected by
template, not by reading the surrounding paragraph.]

## Next steps

Review and edit the generated files to add your voice and institutional details.
Then run `/idstack:course-quality-review` to audit the complete course against QM
standards and CoI presence.

---

*Generated by `/idstack:course-builder`. The system-readable manifest section is in `.idstack/project.json` under `course_content`.*
```
...
**Populate the `course_content` section** (set `report_path` to the report you just wrote):

```json
{
  "course_content": {
    "report_path": ".idstack/reports/course-builder.md",
...
- **Read this:** `.idstack/reports/course-builder.md` — the build report with
  evidence-backed design choices, cognitive-load notes, and a placeholders list.
