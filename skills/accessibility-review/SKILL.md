...
2. Modify ONLY the `accessibility_review` section. Preserve all other sections
   unchanged — `context`, `needs_analysis`, `learning_objectives`, `assessment_design`,
   `course_builder`, `quality_review`, and any other sections must remain exactly
   as they were.
...
Populate the `accessibility_review` section with:
