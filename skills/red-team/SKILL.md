...
The sub-agent (the **orchestrator**) runs the full audit, writes a structured report to `.idstack/red-team-report.md`, and returns a short executive summary. The parent (this skill) then offers to apply fixes in-context, since the parent already knows the course structure and is good at editing.
...
Write `.idstack/red-team-report.md` with this structure (Markdown):
...
- Path: `.idstack/red-team-report.md`
...
1. Read `.idstack/red-team-report.md` (full report).
...
3. Mention: "Full report at `.idstack/red-team-report.md` — open it for the complete finding list."
...
`.idstack/red-team-report.md` (the report is the source of truth — do not re-derive from
...
"$_IDSTACK/bin/idstack-manifest-merge" --section red_team_audit --payload - <<'PAYLOAD'
{
  ...
  "confidence_score": 0,
  "focus": "Full sweep",
  "report_path": ".idstack/red-team-report.md",
...
Two sentences:
- "Confidence: X/100. Applied N fixes (M deferred). Report at `.idstack/red-team-report.md`."
