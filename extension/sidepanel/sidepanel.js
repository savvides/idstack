...
export async function updateDossierUI() {
  ...
  try {
    const dossier = await getDossier();
    listEl.innerHTML = window.DOMPurify.sanitize(renderDossierListHTML(dossier));
    ...
    listEl.innerHTML = window.DOMPurify.sanitize('<div class="dossier-empty"><p>Error loading dossier.</p></div>');
  }
}
...
export function renderResults(data) {
  ...
  container.innerHTML = window.DOMPurify.sanitize(renderAuditHTML(data));
  ...
  container.querySelectorAll('.feedback-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (parent) {
        parent.innerHTML = window.DOMPurify.sanitize('<em>Thank you for your feedback!</em>');
      }
    });
  });
