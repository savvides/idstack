...
- If the user wants to continue, proceed without manifest context. You can still
  ...
  gets created at the end so results persist (`bin/idstack-migrate --init`, see
  Write Manifest) — the merge tool needs one to merge into.
...
approach as the `/idstack:learning-objectives` skill: ask for clarification when verbs are
...
- **Purpose:** Close the gap between current and desired performance
  [Assessment-9] [T5]. Each checkpoint should give students evidence of where they
  stand relative to the rubric criteria.
...
If `.idstack/project.json` does not exist yet — the standalone case, where the user chose to continue without upstream data — create it first with `--init`, which writes a canonical manifest with every section at its default. The merge tool then merges into it (without a manifest it exits 4):
