...
11 skills total. The 8-skill linear pipeline is the spine; `/idstack:course-import` branches in when there's an existing course to start from, and `/idstack:learn` is a cross-project memory utility that runs outside the pipeline. All invoked via `/idstack:<skill>`:
